using UnityEngine;
using UnityEditor;
public class RBDropObjectsEditor : EditorWindow
{
    RaycastHit hit;
    float yOffset;
    int savedLayer;
    ...
    static void Awake()
    {
        EditorWindow.GetWindow<RBDropObjectsEditor>().Show();                         // Get existing open window or if none, make a new one
    }

    void OnGUI()
    {
        GUILayout.Label("Drop RB using: ", EditorStyles.boldLabel);
    ...
    void DropObjects(string Method)
    {
        for (int i = 0; i < Selection.transforms.Length; i++)                       // drop multi-selected objects using the selected method
        {
            ...
            Bounds bounds = go.GetComponent<BoxCollider>().bounds;                     // get the collider's bounds, because we want the collider exactly on the surface
            // This is set for boxcolliders to ensure multiple colliders get the right one
            ...
            if (Physics.Raycast(go.transform.position, -Vector3.up, out hit))       // check if raycast hits something
            {
                ...
                {
                    go.transform.up = hit.normal + UpVector; // Already only hits colliders...
                }
